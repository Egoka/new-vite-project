  ...
  export function openAlert(optionsAlert: IAlert) {
    ...
    function alertClassPosition(position: IAlert["position"] = "center"): Array<string> {
      ...
      if (position.includes("bottom")) {
        arrayClass.push(`bottom-0 mb-5`)
      } else if (position.includes("top")) {
        arrayClass.push(`top-0 mt-5`)
      } else {
        arrayClass.push("top-1/2 -translate-y-1/2")
      }
      if (position.includes("right")) {
        arrayClass.push(`right-0 mr-5`)
      } else if (position.includes("left")) {
        arrayClass.push(`left-0 ml-5`)
      } else {
        arrayClass.push("left-1/2 -translate-x-1/2")
      }
      return arrayClass
    }
  }
...
<template>
  <transition
    ...
    <div
      v-if="isVisible"
      :class="tailwind.cn('alert-body p-4 w-auto max-w-[89vw] rounded-md', style.body, classClass, size)"
      :style="props.style">
      <div class="flex">
        <div class="shrink-0">
        ...
        <div class="ml-3">
          ...
          <div
            v-if="subtitle"
            :class="tailwind.cn('text-sm', !title?.length || 'mt-2', style.subtitle)"
            v-html="subtitle" />
          <div v-if="!!$slots?.default" :class="tailwind.cn('text-sm', !title?.length || 'mt-2', style.subtitle)">
            <slot />
          </div>
        </div>
